...
from typing import Optional, TypeAlias, ForwardRef, get_args, get_origin
...
def is_jsonable_spec(type_):
    ...
    if type_ == "JSONable" or (
        isinstance(type_, ForwardRef) and type_.__forward_arg__ == "JSONable"
    ):
        return True
    args = get_args(type_)
    if set(get_origin(arg) or arg for arg in args) != {
        str,
        int,
        float,
        bool,
    ...
    for arg in args:
        if get_origin(arg) == list and not is_jsonable_spec(get_args(arg)[0]):
            return False
        if get_origin(arg) == MutableMapping and (
            get_args(arg)[0] != str or not is_jsonable_spec(get_args(arg)[1])
        ):
            return False
    return True
...
def is_jsonobject_spec(type_):
    ...
    if type_ == "JSONObject" or (
        isinstance(type_, ForwardRef) and type_.__forward_arg__ == "JSONObject"
    ):
        return True
    if (
...
def is_jsonable(value):
    ...
    if isinstance(value, MutableMapping):
        return all(isinstance(key, str) for key in value.keys()) and all(
            is_jsonable(element) for element in value.values()
        )
    return False
